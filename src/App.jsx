          ...
          <section className="results-section">
            ...
            <div className="sensitivity-table">
              <table>
                ...
                <tbody>
                  {[
                    {label: 'This Site (Stranded Gas)', rate: gasResults.powerCostPerKwh, isCurrent: true},
                    {label: 'ERCOT Grid (avg)', rate: 0.04},
                    {label: 'Texas Commercial', rate: 0.055},
                    {label: 'US Average', rate: 0.07},
                    {label: 'Premium Industrial', rate: 0.09},
                  ].map(({label, rate, isCurrent}) => {
                    const monthlyCost = rate * gasResults.availableMw * 730 * 1000
                    const diff = monthlyCost - (gasResults.powerCostPerKwh * gasResults.availableMw * 730 * 1000)
                    const annualSavings = -diff * 12
                    return (
                      <tr key={label} className={isCurrent ? 'current' : ''}>
                        <td className="row-label">{label}</td>
                        <td>{(rate * 100).toFixed(2)}¢</td>
                        <td>{formatCurrency(monthlyCost)}</td>
                        <td style={{color: isCurrent ? '#e2e8f0' : diff > 0 ? '#ef4444' : '#22c55e'}}>
                          {isCurrent ? '—' : diff > 0 ? `+${formatCurrency(diff)}/mo` : `${formatCurrency(diff)}/mo`}
                        </td>
                        <td style={{color: isCurrent ? '#e2e8f0' : '#22c55e', fontWeight: '600'}}>
                          {isCurrent ? '—' : formatCurrency(annualSavings) + '/yr'}
                        </td>
                      </tr>
                    )
                  })}
            ...
            {/* Power Cost Sensitivity — Load % vs Generator Mode */}
            <h3 style={{marginTop: '32px', marginBottom: '16px'}}>Sensitivity — Effective ¢/kWh by Load %</h3>
            <p className="section-intro">How power cost changes with generator load % across acquisition modes</p>
            <div className="sensitivity-table">
              <table>
                <thead>
                  <tr>
                ...
                <tbody>
                  {[0.6, 0.7, 0.8, 0.9, 1.0].map(load => {
                    const mw = gasResults.fleetCapacityMw * load
                    const kwh = mw * 730 * 1000
                    const rentCost = generatorRentMonthly * generatorCount
                    const rtoCost = generatorRtoMonthly * generatorCount
                    const buyCost = generatorBuyMaintenance * generatorCount
                    const financeCost = gasResults.financeMonthlyPayment + generatorBuyMaintenance * generatorCount
                    const isCurrentLoad = Math.abs(load - generatorLoadPct) < 0.05
                    return (
                      <tr key={load} className={isCurrentLoad ? 'current' : ''}>
                        <td className="row-label">{Math.round(load * 100)}%</td>
                        {[rentCost, financeCost, rtoCost, buyCost].map((cost, i) => {
                          const centsPerKwh = kwh > 0 ? (cost / kwh) * 100 : 0
                          return (
                            <td key={i} style={{color: centsPerKwh < 2 ? '#22c55e' : centsPerKwh < 4 ? '#e2e8f0' : '#ef4444'}}>
                              {centsPerKwh.toFixed(2)}¢
                            </td>
                          )
                        })}
                      </tr>
